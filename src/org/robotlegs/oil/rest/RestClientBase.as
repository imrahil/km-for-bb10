...
package org.robotlegs.oil.rest
{
    ...
    public class RestClientBase implements IRestClient
    {
        ...
        public function RestClientBase(rootURL:String = "", userAgent:String = "")
        {
        ...
        public function del(url:String, params:Object = null):IPromise
        {
        ...
        public function get(url:String, params:Object = null):IPromise
        {
        ...
        public function post(url:String, params:Object = null):IPromise
        {
            const req:URLRequest = new URLRequest(fullUrl(url));
        ...
        public function put(url:String, params:Object = null):IPromise
        {
            // note: do not transform params as this will be done in post
            params ||= {};
            // method override, see: Rack::MethodOverride
        ...
        {
            const promise:IPromise = promises[event.target];
        ...
        protected function handleIoError(event:IOErrorEvent):void
        {
            const promise:IPromise = promises[event.target];
            releasePromise(promise);
            promise.dispatchError({error: "IO Error", message: event.text, event: event});
        }

        protected function handleProgress(event:ProgressEvent):void
        {
            const promise:IPromise = promises[event.target];
            promise.dispatchProgress({bytesTotal: event.bytesTotal, bytesLoaded: event.bytesLoaded, event: event});
        }
        ...
        protected function handleSecurity(event:SecurityErrorEvent):void
        {
            const promise:IPromise = promises[event.target];
            releasePromise(promise);
            promise.dispatchError({error: "Security Error", message: event.text, event: event});
        }

        protected function releasePromise(promise:IPromise):void
        {
        ...
        protected function request(req:URLRequest):IPromise
        {
            req.userAgent = userAgent;
            req.manageCookies = false;

            const promise:IPromise = new Promise();
